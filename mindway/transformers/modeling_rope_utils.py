...
def _compute_yarn_parameters(
    ...
    pos_freqs = base ** (mint.arange(0, dim, 2).float() / dim)
...
def _compute_longrope_parameters(
    ...
    inv_freq_shape = mint.arange(0, dim, 2, dtype=ms.int64).float() / dim
...
def _compute_llama3_parameters(
    ...
    inv_freq_llama = mint.where(wavelen > low_freq_wavelen, inv_freq / factor, inv_freq)
