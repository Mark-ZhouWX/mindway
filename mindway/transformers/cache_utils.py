...
class MambaCache:
    ...
    def __init__(
        ...
        for _ in range(config.num_hidden_layers):
            conv_state: ops.Tensor = ops.zeros((self.max_batch_size, self.intermediate_size, self.conv_kernel_size))
            ssm_state: ms.Tensor = ops.zeros((self.max_batch_size, self.intermediate_size, self.ssm_state_size))
    ...
    def update_conv_state(self, layer_idx: int, new_conv_state: ms.Tensor, cache_position: ms.Tensor) -> ms.Tensor:
        ...
        conv_state[:, :, cache_position] = new_conv_state.to(conv_state.dtype)
        self.conv_states[layer_idx] = mint.zeros_like(self.conv_states[layer_idx])
    ...
    def reset(self):
        for layer_idx in range(len(self.conv_states)):
            # In-place ops prevent breaking the static address
            self.conv_states[layer_idx] = mint.zeros_like(self.conv_states[layer_idx])
            self.ssm_states[layer_idx] = mint.zeros_like(self.ssm_states[layer_idx])
...
class OffloadedStaticCache(StaticCache):
    ...
    Example:
        ...
        >>> # Leave empty space for 10 new tokens, which can be used when calling forward iteratively 10 times to generate
        >>> max_generated_length = inputs.input_ids.shape[1] + 10
        >>> past_key_values = OffloadedStaticCache(config=model.config, max_batch_size=1, max_cache_len=max_generated_length, dtype=model.dtype)
        >>> outputs = model(**inputs, past_key_values=past_key_values, use_cache=True)
    ...
    def __init__(
        ...
        for i in range(config.num_hidden_layers):
            ...
            key_cache, value_cache = self._create_key_value_cache_tensors(cache_shape)

            self.key_cache.append(key_cache)
            self.value_cache.append(value_cache)

        # Create device tensors.
        self._device_key_cache: List[ms.Tensor] = []
        self._device_value_cache: List[ms.Tensor] = []
    ...
    def _create_key_value_cache_tensors(self, shape: Tuple[int, ...], device: None) -> Tuple[ms.Tensor, ms.Tensor]:
        ...
        key_cache = mint.zeros(shape, dtype=self.dtype)
        value_cache = mint.zeros(shape, dtype=self.dtype)
