...
class BeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        next_beam_scores = mint.zeros((batch_size, self.group_size), dtype=next_scores.dtype)
        next_beam_tokens = mint.zeros((batch_size, self.group_size), dtype=next_tokens.dtype)
        next_beam_indices = mint.zeros((batch_size, self.group_size), dtype=next_indices.dtype)
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def step_sentence_constraint(
        ...
        full_hypotheses = mint.cat((input_ids[sent_beam_indices], sent_beam_tokens.unsqueeze(-1)), dim=-1)
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                ...
                for advance_token in advance_tokens:
                    ...
                    advance_seq = mint.cat((pre_seq, advance_token.unsqueeze(0)), -1).asnumpy().tolist()
                ...
                new_score, new_token = mint.max(this_batch_token_scores[seq_idx], 0)  # some next probable token
                advance_seq = mint.cat((pre_seq, new_token.unsqueeze(0)), -1)
        ...
        if len(track_new["new_indices"]) > 0:
            ...
            new_tokens = mint.stack(track_new["new_tokens"])
            new_scores = mint.stack(track_new["new_scores"])

            all_states = topk_contraint_states + track_new["new_states"]
            all_tokens = mint.cat((sent_beam_tokens, new_tokens), -1)
            all_scores = mint.cat((sent_beam_scores, new_scores), -1)
            ...
            sent_beam_indices = mint.cat((sent_beam_indices, new_indices))[indices]
