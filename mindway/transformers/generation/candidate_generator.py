...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    def __init__(
        ...
        if assistant_model.config.is_encoder_decoder:
            ...
            self.assistant_kwargs["attention_mask"] = self.assistant_kwargs.get(
                ...
                mint.ones((input_ids.shape[0], 1), dtype=ms.int64),
            )
        else:
    ...
    def _calculate_new_tokens(self, input_ids: ms.Tensor) -> tuple[int, int]:
    ...
    def _update_past_and_masks(
        self, input_ids: ms.Tensor, remove_from_pkv: int = 0, num_added_tokens: int = 1
    ) -> bool:
    ...
    def _prepare_generation_args(self, input_ids: ms.Tensor, min_new_tokens: int, max_new_tokens: int) -> dict:
    ...
    def _generate_candidates(self, generation_args: dict) -> tuple[ms.Tensor, Optional[ms.Tensor]]:
        ...
        if (
            ...
            scores_tensor = mint.cat(assistant_output.scores, dim=0)
            scores_softmax = mint.softmax(scores_tensor, dim=-1)
            ids = assistant_output.sequences[-1, -len(assistant_output.scores) :]
            p = scores_softmax[range(len(ids)), ids]
            self.probs.extend(p.tolist())
        candidate_logits = mint.stack(assistant_output.scores, dim=1)
...
class AssistedCandidateGeneratorDifferentTokenizers(AssistedCandidateGenerator):
    ...
    def _get_longest_diag_dict(input_matrix, nonzero_idx):
        ...
        Args:
            input_matrix (ms.Tensor): The input matrix.
            nonzero_idx (ms.Tensor): The indices of the non-zero elements in the matrix.
        Returns:
    ...
    def _prepare_assistant_input_ids(self, input_ids: ms.Tensor) -> tuple[ms.Tensor, int]:
        ...
        if self.prev_assistant_ids is not None and self.prev_target_ids_len > self.target_lookbehind:
            ...
            if new_tokens_only is not None:
                ...
                if new_tokens_only.shape[1] > 0:
                    assistant_input_ids = mint.cat([assistant_input_ids, new_tokens_only], dim=-1)
            else:
                # edge case: in case of no intersection between prompt and new_assistant_ids
                assistant_input_ids = mint.cat([assistant_input_ids, new_assistant_ids], dim=-1)
        else:
    ...
    def _process_assistant_outputs(
        ...
        if target_new_tokens_only is not None:
            if target_new_tokens_only.shape[1] > 0:
                new_target_ids = mint.cat([new_target_ids, target_new_tokens_only], dim=-1)
        else:
            # edge case: in case of no intersection between prompt and new_target_ids
            new_target_ids = mint.cat([new_target_ids, new_target_ids_from_window], dim=-1)
...
class AssistantToTargetTranslator:
    ...
    def _get_suppress_input_ids(self) -> list[int]:
        ...
        """
        return mint.where(self._assistant_to_target_input_ids == self.SUPPRESS_TOKEN_ID)[0]

    def get_target_ids(
        self, assistant_input_ids, target_input_ids, assistant_candidate_ids: ms.Tensor
    ) -> ms.Tensor:
        """
...
class UniversalSpeculativeDecodingGenerator(AssistedCandidateGeneratorDifferentTokenizers):
    ...
    def __init__(
        ...
        self._prev_assistant_ids: Optional[ms.Tensor] = None
    ...
    def _update_past_and_masks(self, assistant_input_ids: ms.Tensor, num_added_tokens: int = 1) -> bool:
    ...
    def _prepare_assistant_input_ids(self, target_input_ids: ms.Tensor) -> ms.Tensor:
        """
        ...
        if assistant_new_ids is None:
            ...
            assistant_new_ids = ms.Tensor([[assistant_new_ids]], device=self.assistant_model.device)
...
class PromptLookupCandidateGenerator(CandidateGenerator):
    ...
    def get_candidates(self, input_ids: ms.Tensor) -> tuple[ms.Tensor, Optional[ms.Tensor]]:
        ...
        for ngram_size in range(min(self.max_matching_ngram_size, input_length - 1), 0, -1):
            ...
            for idx in match_indices:
                ...
                if start_idx < end_idx:
                    ...
                    mask = mnp.isin(chosen_ids, self.eos_token_id)
                    match_indices_eos = mint.nonzero(mask)
        ...
        candidate_input_ids = mint.cat((input_ids, chosen_ids), dim=1)
...
def _prepare_attention_mask(model_kwargs: dict[str, Any], new_length: int, is_encoder_decoder: bool) -> dict[str, Any]:
    ...
    if mask_length_diff < 0:
        ...
        model_kwargs[mask_key] = mint.cat([mask, mask.new_ones((mask.shape[0], mask_length_diff))], dim=-1)
    ...
    if "cross_attention_mask" in model_kwargs:
        ...
        if mask_length_diff < 0:
            ...
            model_kwargs["cross_attention_mask"] = mint.cat([cross_mask, new_mask], dim=1)
        ...
        if mask_length_diff < 0:
            ...
            model_kwargs["image_attention_mask"] = mint.cat([cross_mask, new_mask], dim=1)
...
def _prepare_token_type_ids(model_kwargs: dict[str, Any], new_length: int) -> dict[str, Any]:
    ...
    if type_length_diff < 0:
        ...
        model_kwargs["token_type_ids"] = mint.cat([model_kwargs["token_type_ids"], token_type_copies], dim=-1)
    return model_kwargs
