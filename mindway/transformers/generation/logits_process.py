...
from mindway.transformers.mindspore_adapter.utils import dtype_to_min, dtype_to_max
...
class SequenceBiasLogitsProcessor(LogitsProcessor):
    ...
    def _prepare_bias_variables(self, scores: ms.Tensor):
        ...
        # with simpler logic.
        self.length_1_bias = mint.zeros((vocabulary_size,), dtype=ms.float32).to(scores.device)
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        for batch_idx in range(batch_size):
            ...
            token_frequency = mint.bincount(previous_group_tokens, minlength=vocab_size).to(scores.device)
...
class ForcedEOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, max_length: int, eos_token_id: Union[int, List[int], ms.Tensor], device: str = "cpu"):
        ...
        if self.unconditional_context["first_pass"]:
            ...
            if self.unconditional_context["attention_mask"] is None:
                self.unconditional_context["attention_mask"] = mint.ones_like(
                    self.unconditional_context["input_ids"], dtype=ms.int64
                )
            ...
            attention_mask = mint.cat(
                [
                    ...
                    mint.ones_like(input_ids[:, -1:], dtype=ms.int64),
                ],
                dim=1,
            )
...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        self.fixed_table = mint.randperm(self.table_size, generator=self.rng,)
