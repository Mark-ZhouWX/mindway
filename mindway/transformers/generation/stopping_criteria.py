...
class StopStringCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor, **kwargs) -> ms.Tensor:
        ...
        lengths_with_ends = mint.cat([end_lengths, lengths], dim=1)
        ...
        later_match = mint.any(cumsum[:, :-1, :, None] == valid_positions[:, :, :, :, None], dim=-2)

        # The match vector is a boolean vector that indicates which positions have valid tokens
        match = mint.cat([initial_match, later_match], dim=1)
        ...
        string_matches = mint.amax(cumsum * mask, dim=(1, -1)) >= self.target_lens[None, :]

        # We return a per-sample vector that is True if any stop string is matched for that sample
        return mint.any(string_matches, dim=-1)
